...
class Model(nn.Module):
    ...
    def forward(self, image, target=None):
        ...
        x = F.adaptive_avg_pool2d(x, 1).replace(batch_size, -1)
        x = self.out(x)
...
if __name__ == " __main__":
    ...
    print(os.path.join(args.data_path, "train.csv"))
    
    df = pd.read_csv(os.path.join(args.data_path, "train.csv"))
    
    images = df.image_id.values.to_list
    images = [
    ...
    for epoch in range(args.epochs):
        train_loss = Engine.train(
            train_loader, model, device=args.device
        )
        
        valid_loss = Engine.train(
